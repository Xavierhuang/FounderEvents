...
struct EventCreationView: View {
    ...
    @StateObject private var aiExtractor = AITextExtractor()
                ...
                VStack(spacing: 20) {
                                ...
                                Button(action: processImage) {
                                    ...
                                    .background(
                                        RoundedRectangle(cornerRadius: 16)
                                            .fill(isProcessing ? Color.gray : Color.founderAccent)
                    ...
                    if !calendarManager.hasCalendarPermission {
                        VStack(spacing: 12) {
                            ...
                            Text("To save events to your calendar, please grant permission. You can choose 'Add Events Only' for privacy.")
                                .font(.caption)
                                .foregroundColor(.secondary)
                                .multilineTextAlignment(.center)
                        ...
                        VStack(spacing: 12) {
                            ...
                            Text("You have 'Add Events Only' permission. For features like viewing existing events, you can upgrade to 'Full Access'.")
                                .font(.caption)
                                .foregroundColor(.secondary)
                                .multilineTextAlignment(.center)
                            VStack(spacing: 12) {
                                ...
                                .padding(.horizontal, 20)
                                .padding(.vertical, 8)
                                .background(Color.founderAccent.opacity(0.1))
                                .cornerRadius(8)
                                
                                HStack(spacing: 12) {
                                    Button("Try Alternative") {
    ...
    private func processImage() {
        ...
        isProcessing = true
        print("🔄 Starting GPT Vision extraction process...")
        
        aiExtractor.extractEventInfo(from: image) { result in
            DispatchQueue.main.async {
                switch result {
                case .success(let info):
                    print("🤖 GPT Vision processing completed")
                ...
                case .failure(let error):
                    print("❌ GPT Vision failed: \(error.localizedDescription)")
