...
enum TransportationMode: String, CaseIterable, Codable {
    ...
    var color: Color {
        ...
        case .rideshare: return .founderAccent
...
class AppState: ObservableObject {
    ...
    private func saveEventsToLocalStorage() {
        if let encoded = try? JSONEncoder().encode(events) {
            ...
            // Also save to App Groups for share extension access
            saveEventsToAppGroups(encoded)
        }
    }
    
    private func saveEventsToAppGroups(_ encoded: Data) {
        let suiteName = "group.com.HainanMandi.eventsAI"
        guard let sharedDefaults = UserDefaults(suiteName: suiteName) else {
            print("❌ AppState: Failed to access App Groups UserDefaults for saving")
            return
        }
        
        sharedDefaults.set(encoded, forKey: "SharedEvents")
        sharedDefaults.synchronize()
        print("💾 Saved \(events.count) events to App Groups")
    }
    func loadEventsFromLocalStorage() {
        // First try to load from App Groups (same source as share extension)
        if let appGroupEvents = loadEventsFromAppGroups() {
            events = appGroupEvents
            print("📂 Loaded \(events.count) events from App Groups")
            return
        }
        
        // Fallback to local storage if App Groups fails
        if let data = UserDefaults.standard.data(forKey: "savedEvents"),
           let decoded = try? JSONDecoder().decode([CalendarEvent].self, from: data) {
            events = decoded
            print("📂 Loaded \(events.count) events from local storage (fallback)")
        }
    }
    ...
    private func loadEventsFromAppGroups() -> [CalendarEvent]? {
        let suiteName = "group.com.HainanMandi.eventsAI"
        guard let sharedDefaults = UserDefaults(suiteName: suiteName) else {
            print("❌ AppState: Failed to access App Groups UserDefaults with suiteName: \(suiteName)")
            return nil
        }

        sharedDefaults.synchronize()
        guard let data = sharedDefaults.data(forKey: "SharedEvents") else {
            print("🔍 AppState: No 'SharedEvents' data found in App Groups")
            return nil
        }

        do {
            let events = try JSONDecoder().decode([CalendarEvent].self, from: data)
            print("✅ AppState: Loaded \(events.count) events from App Groups")
            return events
        } catch {
            print("❌ AppState: Failed to decode events from App Groups: \(error)")
            return nil
        }
    }
    
    // MARK: - App Groups: LinkedIn Profiles
    /// Loads LinkedIn profiles saved by the share extension from the shared App Group container.
    /// - Returns: Array of `LinkedInProfile` decoded from App Groups storage, or an empty array if unavailable.
    func loadLinkedInProfilesFromAppGroups() -> [LinkedInProfile] {
        let suiteName = "group.com.HainanMandi.eventsAI"
        guard let sharedDefaults = UserDefaults(suiteName: suiteName) else {
            print("❌ AppState: Failed to access App Groups UserDefaults with suiteName: \(suiteName)")
            return []
        ...
        sharedDefaults.synchronize()
        guard let data = sharedDefaults.data(forKey: "LinkedInProfiles") else {
            print("🔍 AppState: No 'LinkedInProfiles' data found in App Groups")
            return []
        }

        do {
            let profiles = try JSONDecoder().decode([LinkedInProfile].self, from: data)
            print("✅ AppState: Loaded \(profiles.count) LinkedIn profiles from App Groups")
            return profiles
        } catch {
            print("❌ AppState: Failed to decode LinkedIn profiles from App Groups: \(error)")
            return []
        }
    }
    
    /// Saves LinkedIn profiles to the shared App Group container.
    /// - Parameter profiles: Array of `LinkedInProfile` to save
    func saveLinkedInProfilesToAppGroups(_ profiles: [LinkedInProfile]) {
        let suiteName = "group.com.HainanMandi.eventsAI"
        guard let sharedDefaults = UserDefaults(suiteName: suiteName) else {
            print("❌ AppState: Failed to access App Groups UserDefaults for saving LinkedIn profiles")
            return
        }
        ...
        do {
            ...
            print("❌ AppState: Failed to encode LinkedIn profiles for App Groups: \(error)")
        }
    }
